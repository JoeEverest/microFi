  ...
  if(in_array($_FILES["file"]["type"],$allowedFileType)){
        ...
        for($i=0;$i<$sheetCount;$i++)
        {
            ...
            foreach ($Reader as $Row)
            {
                ...
                $groupname = "";
                if(isset($Row[5])) {
                    $groupname = mysqli_real_escape_string($conn,$Row[5]);
                }
                if (!empty($name) || !empty($description)) {
                    $query = "insert into microfiUpload(customer_id,customer_name,date,description,amount,group_name) values('".$id."','".$name."','".$date."','".$description."','".$amount."','".$groupname."')";
...
<?php
    $sqlSelect = "SELECT * FROM microfiUpload";
